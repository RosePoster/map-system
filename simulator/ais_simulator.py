...
class RouteAisPublisher(BaseMqttPublisher):
    def __init__(self, config: MqttConfig, routes: List[VesselRoute], interval: float) -> None:
        super().__init__(config)
        self.routes = routes
        self.interval = interval

    def before_loop(self) -> None:
        print(f"Publishing continuous routes to {self.config.topic} every {self.interval:.1f}s")
...
def main() -> None:
    ...
    publisher = RouteAisPublisher(config, build_routes(), args.interval)
