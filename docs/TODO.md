...
- **显式实现 Agent Loop (P3)**：利用 `LlmClient` 的 function calling 支持，让 LLM 能够主动查询态势和触发计算；首步限定为只读查询与受控推理，不直接拥有执行权限。
- **法律法规与历史危险场景 GraphRAG 接入 (P4)**：围绕法规条文、避碰规则（COLREGS）与历史危险场景建立实体关系图谱，基于当前风险场景检索相关子图，为解释与建议提供规则依据、相似案例参考与可追溯推理链。
- **结构化 Advisory 输出 (P3)**：将文本风险解释升级为包含语义标签、置信提示、动作建议、可视化联动指令的结构化输出；schema 与 `EVENT_SCHEMA.md` 分离，不直接生成可执行坐标路径。
...
- **Advisory schema 分层决策**：明确结构化 advisory 是后端内部结构，还是作为前端可消费契约；同时确定其应扩展 `EXPLANATION` 还是定义独立事件类型。
...
- **前端增强模块 v0.9**：在两天窗口内统一交付前端测试、聊天交互增强与风险展示增强，并在完成后归档实施记录。
- **前端文档完善**：补全前端必要文档；
- **前端测试基线**：补齐前端自动化测试最小闭环；优先覆盖 `store/`、`services/`、协议解析与关键交互流（SSE 连接、WS 回复、语音输入状态切换），并建立至少一条 dashboard 冒烟用例。
- **`risk_score` 视觉化**：将当前仅用于排序的 `risk_score` 升格为前端次级展示信号，用于卡片强度、辅助条或渐变提示，但不改变 `risk_level` 阈值与告警语义。
- **取消录音按钮**：在本地录音进入 `recording` 状态后提供显式取消入口，允许用户撤销误触发或说错的录音流程，直接丢弃本次录音且不发送后端。
...
- **从某条消息重新编辑**：允许用户从指定历史消息恢复输入并重新发送，明确后续消息的保留、截断或重建规则。
