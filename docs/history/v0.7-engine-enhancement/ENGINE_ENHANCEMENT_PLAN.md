...
> 最后更新：2026-04-14
> 后续演进方向（CT 模型、D-S 融合、航道约束等）已提取至 [`docs/TODO.md`](../TODO.md)。
...
Step 2  目标船航迹预测（CV 模型）              ← 航迹外推轨迹，为前端可视化和 Step 4A 提供输入
Step 3  会遇态势识别                          ← 对遇/追越/交叉分类
Step 4A 多因子风险评估增强                     ← 域侵入 + pairwise 预测 CPA + 会遇修正 → 综合风险分
...
| Step 4A | 大（重构 RiskAssessmentEngine + pairwise 预测 CPA） | Step 1, 2, 3 |
...
```
map_service/
  source/ais/      ← mqtt/ + config/（AIS 数据入口与解析配置）
  tracking/        ← store/（多源融合目标状态存储）
  chart/           ← api/S57Controller + repository/S57TileRepository（航图服务）
  risk/            ← api/RiskSseController + engine/ + pipeline/ + event/ + transport/risk/ + config/ + scheduling/（风险评估核心）
  llm/             ← 保持现有内部结构不变
  shared/          ← domain/ + dto/ + transport/protocol/ + util/ + config/（跨域共享类型与配置）
```

### 做什么

1. **新建顶层包**：`source/`、`tracking/`、`chart/`、`risk/`、`shared/`
2. **迁移**（纯包声明 + import 变更，不改逻辑，详细指导见 `engine-enhancement-step7.md`）：
   - `mqtt/*`、AIS相关配置 → `source/ais/`
   - `store/*` → `tracking/store/`
   - `repository/*`、`api/S57Controller` → `chart/`
   - `engine/*`、`pipeline/*`、`event/*`、`transport/risk/*`、风险相关配置与调度、`api/RiskSseController` → `risk/`
   - `domain/*`、`dto/*`、其余 `config/*`、`transport/protocol/*`、`util/*` → `shared/`
...
> **范围限定**：本轮安全领域**仅为本船建模**，不涉及目标船域。域侵入检测（Step 4A）采用 own-ship-centric 视角：判断目标船是否进入本船安全领域。该能力在本轮规划范围内不涉及目标船尺寸/船型信息。
...
- 左右可不对称：按内河靠右航行惯例，左舷（来船侧）半径理论上 > 右舷半径；**Step 1 默认值与现网硬编码对齐（port=stbd=0.2 nm），不在首步引入非对称；待模型验证稳定后通过配置调优**
...
2. **新增 `ShipDomainProperties`**
   ...
   public class ShipDomainProperties {
       // Step 1 默认值与现网硬编码对齐；port/stbd 暂保持对称，待模型验证后调优
...
- 单元测试：AIS sentinel SOG（102.3 kn）、负值 SOG、NaN SOG 均回退为基准值（speedFactor = 1.0），不抛异常
...
Step 1 的 `ShipDomainProperties`（`baseForeNm` 等）使用**绝对海里值**，仅适用于本船。这是在本轮规划范围限定（own-ship-centric）下的有意选择，成立前提是本船尺寸已知且相对稳定。
...
实现恒速（Constant Velocity）航迹预测模型，输出**每艘目标船**未来 N 分钟内的预测轨迹点序列，用于前端航迹可视化；同时建立 `ShipTrajectoryStore` 历史存储，作为 Step 4A pairwise 预测 CPA 计算、Step 4B CTR 模型升级和 Step 5B 运动学校验的基础设施。

> **接口边界约定**：`CvPredictionResult` 只描述单船轨迹（per-target），不涉及任何双船关系量。预测 CPA（pairwise 最小距离）是两船关系量，由 Step 4A 的 `PredictedCpaCalculator` 负责，在消费 `CvPredictionResult` 时按需计算。
...
**`MathUtils.clamp()` 提取时机**：Step 1 的 `ShipDomainEngine` 已内联 `Math.min(Math.max(...))` 一处。Step 2 的 `CvPredictionEngine` 在处理 SOG/COG 异常值时会出现第二处。两处共存时应提取 `MathUtils.clamp(double value, double min, double max)`，放入现有 `util/` 包，并同步替换 Step 1 的内联写法。不要在 Step 2 内再次内联。
...
`encounter_type` 字段已在 Step 3 确认，Step 4A 中复用。`risk_score` 首版作为辅助字段，前端用于同级别目标排序，不替换 `risk_level` 的警告触发逻辑。**现在确认：以上字段进入 `RiskObjectDto`。**
...
升级 `CvPredictionEngine` 为 CTR（Constant Turn Rate）模型，利用 `ShipTrajectoryStore` 历史数据提取转向率，使预测轨迹能反映实际机动趋势，消除 `PredictedCpaCalculator` 与原始 DCPA 的冗余性；同时将预测轨迹坐标通过协议传到前端，替换 mock 数据，渲染真实弧线；`TargetsPanel` 消费 `risk_score` 做同级别目标二次排序。

### 做什么

1. **`TrajectoryPredictionProperties`**：追加 `rotThresholdDegPerMin`（转向率噪声阈值，默认 1.5°/min）
2. **`CvPredictionEngine`**：签名增加 `List<ShipStatus> history` 参数；新增 ROT 提取（线性回归，COG 解缠，≥3 有效点）；ROT 高于阈值时使用 Euler 积分 CTR 轨迹，否则退化 CV
3. **`ShipDispatcher.batchPredict()`**：传入 `shipTrajectoryStore.getHistory(targetId)`
4. **协议**：新增 `target.predicted_trajectory: PredictedTrajectoryPoint[]`（可选）
5. **`TargetAssembler`**：填充 `predicted_trajectory` 字段
6. **前端 `MapContainer.tsx`**：替换 `mockDataGenerator` mock 轨迹为真实 `predicted_trajectory`，PathLayer 渲染
7. **前端 `TargetsPanel.tsx`**：同级别目标按 `risk_score` 降序二次排序

### 影响范围

- 修改 `CvPredictionEngine`：CTR 分支 + ROT 提取逻辑
- 修改 `TrajectoryPredictionProperties`：追加阈值字段
- 修改 `ShipDispatcher.batchPredict()`：传入 history
- 修改 `TargetAssembler`：填充轨迹字段
- 修改 `MapContainer.tsx`、`TargetsPanel.tsx`
- 新增 `PredictedTrajectoryPoint` 接口（`schema.d.ts`）

### 协议影响

新增可选字段 `target.predicted_trajectory: PredictedTrajectoryPoint[]`。`PredictionType` 枚举不扩展。

### 验证方式

- 单元测试：已知 ROT 历史序列 → 轨迹各点与手算 CTR Euler 值一致
- 单元测试：历史 < 3 点或 ROT 低于阈值 → 退化直线；COG 跨 0°/360° 解缠正确
- 集成验证：前端目标船轨迹在转向场景下呈弧线；无数据时不报错
- 集成验证：`TargetsPanel` 同级别目标按 `risk_score` 排序生效
...
- 修改 `RiskAssessmentEngine`：消费 confidence（可与 Step 4A 同步实施）
...
- **前端协议新增字段**（已在各 Step 协议影响中确认）：
  ...
  - `target.predicted_trajectory`（Step 4B）
  - `target.risk_assessment.risk_score`（Step 4A）
  - `target.risk_assessment.risk_confidence`（Step 4A）
