...
**11. `LlmRiskContextAssembler` 注入分类器并保持现有参考轴策略**
...
**(11b) `buildTargetContexts()` 中调用分类器并写入会遇类型：**
...
**(11c) `resolveRelativeBearingDeg()` 保持 HDG 优先、COG 回退：**
...
> 与 `EncounterClassifier` 的 COG 几何参考轴不同，`LlmRiskContextAssembler` 保持船体朝向语义：优先 HDG，缺失时回退 COG。该策略与当前实现及测试一致。
...
- **参考轴语义分工**：`EncounterClassifier` 以 COG 为参考轴用于碰撞几何分类；`LlmRiskContextAssembler.resolveRelativeBearingDeg()` 保持 HDG 优先、COG 回退，用于语言描述中的船体方位语义。两者并行存在，互不替代。
