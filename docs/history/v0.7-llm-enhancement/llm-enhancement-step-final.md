...
本步引入内部事件 `RiskAssessmentCompletedEvent`，用于在风险计算完成后向 LLM 模块交付快照。事件负载仅包含 LLM 监听器生成上下文所需的事实数据，不包含任何 LLM 类型；其中 `snapshotVersion` 由 `RiskStreamPublisher` 在单线程发布序列内分配，再随事件一并传递给监听器。字段如下：
...
本步修复原则如下：
- 风险快照在真正进入 `RiskStreamPublisher` 单线程发布序列时获得单调递增的 `snapshotVersion`
...
- 由 `RiskStreamPublisher` 在真正广播 `RISK_UPDATE` 前，先分配该版本号并执行对应的上下文更新回调，再广播该版本的风险帧
...
- 接收由 publisher 分配的 `snapshotVersion`
- 更新 `RiskContextHolder`
...
该顺序保证：
- 在服务端发布序列内，`RiskContextHolder` 中可读到的版本不会领先于尚未进入同一发布序列的风险帧
- 若版本 B 在队列中排在版本 A 之后，则上下文更新和 SSE 广播都严格按 A -> B 顺序生效
- 该保证的边界是服务端内部顺序一致性，不包含客户端已接收确认；SSE 发送完成与前端实际消费之间仍受网络传输和浏览器调度影响
...
- 并发测试：
  ...
  - 验证 `RiskContextHolder` 的最终版本与 publisher 序列中最后一条 `RISK_UPDATE` 对应版本一致
  - 验证较新的上下文不会在较旧风险帧仍未进入同一 publisher 发布点前提前对 chat 可见
