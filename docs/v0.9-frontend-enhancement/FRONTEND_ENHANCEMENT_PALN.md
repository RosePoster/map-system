...
> 最后更新：2026-04-15
> 后续演进方向（agent loop、Generative UI、任意轮次重答等）已提取至 [`docs/TODO.md`](../TODO.md)。
...
3. Chat 交互流存在多处 UX 缺口（取消录音、重答），优先修补交互完备性中改动量小的条目。
...
- 任意轮次重答（依赖消息 ID 体系 + 精确历史截断，保留为第二阶段预留）
...
Step 4  交互增强                                ← 取消录音 + 重答最后一轮 + 解释上下文注入
```
...
修补现有交互流的完备性缺口，实现"重答最后一轮"能力，并将解释文本纳入 `selectTarget` 的上下文注入范围。
...
**实现**：重答最后一轮（`regenerate=true`，作用于最近一组 USER/ASSISTANT 对话）。
...
- 用户发起重答请求
- 前端保留当前最后一轮旧的 `USER/ASSISTANT` 作为回退基线
- 后端以“最后一轮”为目标重新生成回答
- 新回答成功后，再原子替换当前会话最后一组旧的 `USER/ASSISTANT`
- 若新回答失败，前端展示失败态，并允许用户执行“查看上一个回答 / restore previous answer”
- 不保留历史版本树；失败时可恢复的旧回答仅作为轻量级回退分支，不作为长期并存的对话分支
...
interface ChatRequestPayload {
  ...
  regenerate?: boolean;           // 新增：是否重答最后一轮
}
```

- 普通请求：`regenerate` 不传或为 `false`，按现有流程追加会话
- 重答请求：`regenerate: true`，作用于最后一轮
...
5. 新回答失败时，不销毁旧消息，返回失败结果，由前端决定展示失败态还是恢复旧回答

**接口扩展性**：

- 第一阶段：`regenerate=true` 表示"重答最后一轮"
- 第二阶段预留：后续如需按指定轮次重答，扩展 `regenerate_target_message_id` 字段；不建议用单一字段特殊值长期承载全部语义

**前端实现**：

- 在最后一条 `assistant` 消息底部增加"重答"按钮
- 点击后：预填上一条 user 消息内容到输入框，供用户修改后发送；或直接以原内容重发
- `sendTextMessage` 新增 `regenerate?: boolean` 参数，透传至 WS 上行 payload
- 前端 store 处理：
  - 重答请求发送后，不立即销毁旧的最后一组 user+assistant，而是将其标记为“待替换”
  - 成功回包后，再以新一组 user+assistant 替换旧内容
  - 失败回包后，展示“重答失败”状态，并提供“查看上一个回答 / restore previous answer”入口
  - “restore previous answer” 仅在本次重答失败后可见，不形成长期分支 UI

**后端实现**（v0.9 子步骤，不独立拆分）：

- `ChatWebSocketHandler` 解析 `regenerate` 字段，透传至 `LlmChatService`
- `ConversationMemory` 新增“读取最后一轮 / 成功后替换最后一轮”的能力；不采用“先删后生成”的破坏式路径
- `LlmChatService` 在 `regenerate=true` 时按“先生成、成功后替换、失败则保留旧轮次”的语义执行
- 如协议层需要更清晰区分普通失败与重答失败，可为前端补充失败原因字段或沿用现有 `ERROR`，由前端根据请求上下文决定是否显示恢复入口

**第二阶段预留**：

- 为消息引入稳定 `messageId`
- 前端点击某条历史消息时传递目标消息 ID
- 后端从该消息开始截断其后整段历史，再基于截断后上下文生成新的后续回答

**复杂度判断**：

- 只重答最后一轮：中等复杂度。为避免失败时丢失旧轮次，需引入“成功后替换”的非破坏式语义和前端回退 UI
- 任意轮次重答：中等偏高复杂度，主要成本在消息 ID、协议扩展、精确截断定位

#### 必要性

对当前项目而言，"只重答最后一轮"有实现价值：

- 当前场景是实时海事风险解释，用户问题通常是短链路问答
- 高频需求更像是"上一条回答不满意，重新答一次"
- 实时风险上下文会变化，越早的历史轮次越容易与当前态势脱节
- 任意轮次回滚会引入历史时态与当前风险快照不一致的问题

对 agent loop 场景：如果未来演进为长对话、多步骤任务编排，任意轮次重答价值才会上升；当前阶段，先实现"重答最后一轮"，接口预留扩展能力。
...
- 新增“最近解释缓存”能力，按 `targetId` 持有每个目标最近一条解释文本，并记录其 `riskObjectId` / 时间戳
- `LlmRiskEventListener` 在发布 `EXPLANATION` 时同步更新该缓存
- `RiskContextFormatter` 或上下文注入层：在格式化选中目标数据时，检查缓存中是否存在该目标的最近解释文本
- 存在时追加到上下文中（如："该目标此前的 AI 评估：{explanation_text}"）
- 注入时优先匹配当前风险快照；若解释明显滞后于当前快照，则允许降级为“不注入解释，只注入风险数据”
- 不影响未选中目标的上下文注入行为
...
- 最后一条 assistant 消息底部有"重答"按钮，点击后发起重答请求（`regenerate: true`）
- 重答成功后，旧的最后一轮被新结果替换；重答失败后，用户可执行“查看上一个回答 / restore previous answer”
...
| Step 4 | 取消录音（前端本地）、重答最后一轮（前后端联动，失败可恢复旧回答）、解释上下文注入（后端缓存支持） |
...
| 任意轮次重答 | 依赖消息 ID 体系 + 精确历史截断，复杂度中等；当前场景需求有限 | Step 4B 第二阶段预留 |
