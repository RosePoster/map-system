...
> **范围限定**：本轮安全领域**仅为本船建模**，不涉及目标船域。域侵入检测（Step 4A）采用 own-ship-centric 视角：判断目标船是否进入本船安全领域。若后续需要双船责任判断（give-way/stand-on 基于域侵入），需另行引入目标船尺寸/船型信息；该能力不在本轮规划范围内。
...
若后续需要对目标船建立安全领域（例如为 give-way/stand-on 判定提供双船责任依据），该模型**不可直接复用**——10 米快艇与 130 米货船共享相同绝对领域在物理上是错误的。届时应引入以船长（LOA）为基底的相对参数（如 `fore = N × LOA`），并从 `ShipStatus` 读取真实船舶尺寸。本轮不实施，记录于此以防后续遗漏。
