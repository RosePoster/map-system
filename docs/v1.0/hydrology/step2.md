...
- [`RiskObjectMetaAssembler`](../../../backend/map-service/src/main/java/com/whut/map/map_service/risk/pipeline/assembler/riskobject/RiskObjectMetaAssembler.java) 目前只注入 `safety_contour_val / active_alerts / weather`，且 `buildEnvironmentContext()` 不接收 `ownShip`
- [`RiskObjectAssembler`](../../../backend/map-service/src/main/java/com/whut/map/map_service/risk/pipeline/assembler/RiskObjectAssembler.java) 直接调用无参 `buildEnvironmentContext()`，因此目前没有位置相关环境查询的挂点
...
因此 Step 2 的真实改动面包括：风险对象装配签名调整、hydrology 查询服务、运行时 safety contour 状态、共享告警枚举、前端类型与 slider 写回链路，而不是单点补字段。
...
- `RiskObjectMetaAssembler.buildEnvironmentContext(...)` 改为接收：
  - 生效 safety contour 值
  - hydrology 摘要
  - weather 快照或 weather-derived alerts
...
2. **最近浅区距离**
   ...
   - 若本船已在浅区内，则 `nearest_shoal_nm = 0`
   - 否则返回本船点到最近浅区边界的距离（nm）
...
- 提供线程安全的 `getCurrentDepthMeters()` / `updateDepthMeters(double)` 接口
- 由 `S57Controller`、`RiskObjectMetaAssembler` 上游装配逻辑与 `HydrologyContextService` 共用

Step 2 不直接修改 `application.properties` 的静态配置，也不把运行时写回持久化到磁盘。

### 6.3 风险对象装配链调整

当前 `RiskObjectMetaAssembler.buildEnvironmentContext()` 无法读取本船位置。Step 2 需要把 hydrology 查询前移到 `RiskObjectAssembler`：

1. `RiskObjectAssembler` 读取运行时 safety contour 值
2. 以 `ownShip` 调用 `HydrologyContextService`
3. 把 hydrology 摘要与 contour 值传给 `RiskObjectMetaAssembler`
...
- `bearing_deg` 采用 `0..359` 整数或一位小数均可，但文档与实现需统一
...
若后端更新失败，前端回退到最近一次 SSE 生效值，并给出错误提示。
...
- 前端协议与 slider 消费：
  ...
  - [`RiskExplanationPanel.tsx`](../../../frontend/src/components/Dashboard/RiskExplanationPanel.tsx)
