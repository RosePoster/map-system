...
- 后端已具备 weather ingest、`WeatherContextHolder` 与气象 DTO
...
因此，weather track 已从“从零起步”进入“Step 1 完成、Step 2/3 待继续”的状态。与已有海图与 ENC 基础链路的 hydrology track（参见 [`../hydrology/HYDROLOGY_PLAN.md`](../hydrology/HYDROLOGY_PLAN.md) §1.1）相比，两者的工程起点曾经不同，但目前都已进入“基础视觉链路已落地”的阶段。
...
天气 track 不阻塞 `v1.0` 主版本完成（主版本收口以 agent 主线为准）。Step 1–3 已属于当前规划链，因而不写入 [`../../TODO.md`](../../TODO.md)；若 `v1.0` 关闭时 Step 2 / Step 3 仍未完成，应先迁移到后续 milestone / step 链，只有失去明确 owner 的剩余项才回收至 TODO。
...
- **fog overlay**：作为 Step 1 已完成的基础天气可视化入口
...
- 前端 advisory card 如需补充气象事实项展示，应仅承接 advisory 消费所需的最小 UI 变更；增强天气视觉效果不归入 Step 3
...
- [`../../TODO.md`](../../TODO.md) 仅跟踪未挂入 Step 1–3 的后续 backlog；weather 主线事项不重复记录于 TODO。当前已回收到 TODO 的未挂载项包括真实气象源接入，以及气象历史回放 / 导出 / 多源融合
...
- **下游消费方**：`RiskAssessmentEngine`（Step 2）、`LlmRiskContext` / agent `AgentToolRegistry`（Step 3）、前端天气展示入口（Step 1）与 [`../visual/VISUAL_UPGRADE_PLAN.md`](../visual/VISUAL_UPGRADE_PLAN.md) 承接的增强天气视觉效果
