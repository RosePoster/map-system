  ...
  return (
    <div className="bg-slate-950/60 backdrop-blur-md rounded-md p-3 text-white w-[280px] border border-white/10 shadow-xl pointer-events-auto max-h-[40vh] flex flex-col">
      <div className="flex items-center justify-between border-b border-white/10 pb-2 mb-2">
        <h2 className="text-sm font-bold tracking-wide text-slate-100">目标追踪</h2>
      ...
      <div className="overflow-y-auto pr-1 space-y-1.5 scrollbar-thin">
        {sortedTargets.map((target) => {
          ...
          const hasLlmExplanation = isLlmExplanation(
            isSelected ? selectedTargetLlmExplanation : target.risk_assessment.explanation,
          );
          ...
          return (
            ...
            >
              ...
              {hasLlmExplanation && (
                <div className="mt-2 flex items-center justify-between rounded border border-cyan-500/20 bg-slate-900/70 px-2 py-1">
                  <span className="text-[10px] text-cyan-200">有大模型说明</span>
                  <span className="text-[10px] text-slate-300">LLM</span>
                </div>
              )}
            </div>
          );
...
function isLlmExplanation(explanation?: { source?: string; text?: string } | null): boolean {
  return Boolean(explanation?.text?.trim() && (explanation.source || '').toLowerCase() === 'llm');
}
